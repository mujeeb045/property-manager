...
// ========================================
// Allocate Tenant (Multiple Units Support)
// ========================================
  ...
  try {
    ...
    result.rows.forEach(t => {
      ...
      if (unitList.length > 0) {
        unitsHTML = `<div class="mt-3"><strong>Units:</strong>`;
          ...
          unitsHTML += `
            <div class="flex justify-between items-center bg-gray-50 px-3 py-2 rounded-lg mt-1">
              ...
              <form action="/delete-tenant/${t.id}/${unitIdList[index]}" method="POST" 
                    onsubmit="return confirm('Move out from ${unitName}?');">
    ...
    res.status(500).render('tenants/manage-profiles', { 
      title: 'Manage Active Tenants',
      rowsHTML: '',
      error: 'Error loading tenant profiles'
    });
  }
});

// ========================================
// Assign Additional Unit to Existing Tenant
// ========================================
router.post('/assign-unit', async (req, res) => {
  try {
    ...
    if (!tenantId || !unitId) {
      return res.status(400).send("Tenant and Unit are required");
    }
    ...
    console.error(err);
    res.status(500).send("Error assigning unit to tenant");
  }
});
...
router.post('/update-tenant/:id', async (req, res) => {
  try {
    ...
    await pool.query(`
      UPDATE tenants 
      SET name = $1, father_name = $2, phone = $3, alt_phone = $4, 
          id_card_no = $5, security_deposit = $6
      WHERE id = $7
...
router.post('/delete-tenant/:tenantId/:unitId', async (req, res) => {
  try {
    ...
    // 2. Mark this specific unit as vacant
    await pool.query(`UPDATE units SET is_occupied = FALSE WHERE id = $1`, [unitId]);

    // 3. Check if tenant has ANY remaining active units
    const activeUnits = await pool.query(`
      SELECT COUNT(*) as count 
      FROM tenant_units 
      WHERE tenant_id = $1 AND is_active = TRUE
    `, [tenantId]);

    // 4. Only mark tenant as inactive if they have NO active units left
    if (parseInt(activeUnits.rows[0].count) === 0) {
      await pool.query(`
        UPDATE tenants 
    ...
    res.status(500).send("Error moving out from unit");
  }
});
