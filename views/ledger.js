  ...
  return `
    <h1>
      ...
      <div class="top-nav-box">
        ...
        <form method="GET" action="/tenants" style="margin:0; display:flex; align-items:center; gap:8px;">
          <select name="month" onchange="this.form.submit()" style="margin:0; padding:6px 12px; width:130px; font-size:13px;">
            ${dropdownOptions}
          </select>
        </form>
      </div>
    ...
    <div class="flex-stats">
      <div class="stat-card stat-card-paid">
        <small>Total Collected (${selectedMonth})</small>
        <h2>₹${grossCollected.toLocaleString('en-IN')}</h2>
      </div>

      <div class="stat-card stat-card-unpaid">
        <small>Outstanding Dues (${selectedMonth})</small>
        <h2>₹${grossOutstanding.toLocaleString('en-IN')}</h2>
      </div>
    </div>
    ...
    >
      ${
        tenantRows ||
        `<li style="color:#64748b;text-align:center;padding:40px;">
          No generated statements found for ${selectedMonth}.
        </li>`
      }
    </ul>

    <div style="margin-top:25px;border-top:1px solid #e2e8f0;padding-top:20px;">
      <a href="/" class="btn btn-secondary">← Back to Dashboard Input</a>
    </div>
      ...
      function filterTenants() {
        ...
        for (let i = 0; i < items.length; i++) {
          ...
          items[i].style.display =
            dataSearch.toLowerCase().includes(filter)
              ? ''
              : 'none';
        }
      }
